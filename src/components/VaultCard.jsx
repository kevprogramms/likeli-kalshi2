...
function VaultCard({ vault, onClick, style }) {
    ...
    const getChainBadge = () => {
        if (vault.isHybrid || vault.chain === 'Hybrid') {
            return <span className="chain-badge hybrid">🔗 Hybrid</span>
        } else if (vault.chain === 'Solana') {
            return <span className="chain-badge solana">◎ Solana</span>
        } else {
            return <span className="chain-badge polygon">⬡ Polygon</span>
        }
    }
