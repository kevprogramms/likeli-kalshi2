...
// Demo markets for testing (same as in Markets page)
const DEMO_MARKETS = [
    { id: '1', title: 'Will Bitcoin reach $100k by end of 2025?', category: 'Crypto', probability: 0.65 },
    { id: '2', title: 'Will the Fed cut rates in Q1 2025?', category: 'Economics', probability: 0.42 },
    { id: '3', title: 'Will SpaceX Starship reach orbit in 2025?', category: 'Technology', probability: 0.78 },
    { id: '4', title: 'Will there be a TikTok ban in the US?', category: 'Politics', probability: 0.35 },
];
...
function AIResearcher() {
    ...
    const handleMarketSelect = (market) => {
        setSelectedMarket(market);
        navigate(`/ai-researcher/${market.id}`);
        runAnalysis(market);
    };
    ...
    return (
        <div className="ai-researcher animate-fade-in-up">
            ...
            {/* Market Selector (if no market selected) */}
            {!selectedMarket && (
                <div className="market-selector-section">
                    <h2>Select a Market to Analyze</h2>
                    <div className="market-selector-grid">
                        {DEMO_MARKETS.map((market, i) => (
                            <button
                                key={market.id}
                                className={`market-select-card glass animate-fade-in-up stagger-${i + 1}`}
                                onClick={() => handleMarketSelect(market)}
                            >
                                <span className="market-category">{market.category}</span>
                                <span className="market-title">{market.title}</span>
                                <span className="market-prob">{Math.round(market.probability * 100)}%</span>
                            </button>
                        ))}
                    </div>
                </div>
            ...
            {error && (
                <div className="error-state glass">
                    ...
                    <p>{error}</p>
                    <button onClick={() => runAnalysis(selectedMarket)}>Try Again</button>
                </div>
            )}
                ...
                <div className="analysis-layout">
                    ...
                    <div className="summary-panel glass">
                        ...
                        <div className="probability-section">
                            ...
                            <div className="probability-label">Predicted Probability</div>
                        </div>
