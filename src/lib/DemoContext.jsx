...
export function DemoProvider({ children }) {
    ...
    const depositToVault = (vaultId, amount) => {
        ...
        setVaults(prev => prev.map(vault => {
            ...
            depositFeeCharged = result.depositFee
            depositSuccess = true
            ...
            // Note: Validation of amount ensures it's a number.
            const depositVal = parseFloat(amount)

            updatedInvestors['demo-user'] = {
                ...userInvestment,
                shares: toShares(userInvestment.shares + result.sharesToMint),
                depositedUsdc: toUSDC(userInvestment.depositedUsdc + depositVal),
            }

            const updatedManagerFees = {
                ...vault.managerFeesCollected,
                depositFees: toUSDC((vault.managerFeesCollected?.depositFees || 0) + result.depositFee),
                totalCollected: toUSDC((vault.managerFeesCollected?.totalCollected || 0) + result.depositFee),
            }
    ...
    const requestWithdrawal = (vaultId, shares) => {
        ...
        if (!validation.valid) return { success: false, error: validation.error }
        const sharesToWithdraw = validation.value

        const vault = vaults.find(v => v.id === vaultId)
        if (!vault) return { success: false, error: 'Vault not found' }

        // Spec: Requests allowed in Open or Trading or Settlement
        // (Closed stage uses Redeem)
        if (vault.stage === STAGE.CLOSED) {
            return { success: false, error: 'Vault is Closed. Use Redeem.' }
        }

        const userInvestment = vault.investors['demo-user'] || { shares: 0 }

        // LOGIC CHANGE: Deduct from user shares immediately (Escrow)
        ...
        const request = {
            ...
            sharesFilled: 0,
            usdcReceived: 0,
            // NO navPerShareAtRequest - removed per spec
        ...
        setVaults(prev => prev.map(v => {
            ...
            return {
                ...v,
                pendingWithdrawalShares: toShares(v.pendingWithdrawalShares + sharesToWithdraw),
                escrowedShares: {
                    ...v.escrowedShares,
                    // Add to vault's escrow bucket for this user
    ...
    const executeTrade = (vaultId, marketId, marketName, side, amount, marketResolutionDate = null) => {
        ...
        if (!validation.valid) return { success: false, error: validation.error }
        const tradeAmount = validation.value
        ...
        setVaults(prev => prev.map(vault => {
            ...
            if (tradeAmount > vault.vaultUsdc) {
                result = { success: false, error: 'Insufficient vault balance' }
                return vault
            }
            ...
            let price = 0.5;
            if (vault.trades && vault.trades.length > 0) {
                const lastTrade = vault.trades[vault.trades.length - 1];
                const change = 1 + (Math.random() * 0.1 - 0.05);
                price = Math.max(0.01, Math.min(0.99, lastTrade.price * change));
            } else {
                price = 0.3 + Math.random() * 0.4;
            }

            const sharesAcquired = tradeAmount / price
