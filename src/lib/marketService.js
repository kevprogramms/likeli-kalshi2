...
class MarketService {
    ...
    /**
     * Get vault groups (hybrid Polygon + Solana vaults)
     */
    async getVaultGroups() {
        try {
            const response = await fetch('http://localhost:3001/api/vault-groups');
            if (!response.ok) throw new Error('Failed to fetch vault groups');
            return await response.json();
        } catch (error) {
            console.error('Error fetching vault groups:', error);
            return [];
        }
    }
    ...
    async createVaultGroup(data) {
        try {
            ...
            if (!response.ok) throw new Error('Failed to create vault group');
            return await response.json();
        } catch (error) {
            console.error('Error creating vault group:', error);
            throw error;
        }
    }
    ...
    async getEtfBaskets() {
        try {
            const response = await fetch('http://localhost:3001/api/etf');
            if (!response.ok) throw new Error('Failed to fetch ETF baskets');
            return await response.json();
        } catch (error) {
            console.error('Error fetching ETF baskets:', error);
            return [];
        }
    }

    /**
     * Get single ETF basket by ID
     */
    async getEtfBasket(id) {
        try {
            const response = await fetch(`http://localhost:3001/api/etf/${id}`);
            if (!response.ok) throw new Error('Failed to fetch ETF basket');
            return await response.json();
        } catch (error) {
            console.error('Error fetching ETF basket:', error);
            return null;
        }
    }
     ...
     * Mint ETF shares
     */
    async mintEtfShares(basketId, userId, amount, txHash) {
        try {
            const response = await fetch(`http://localhost:3001/api/etf/${basketId}/mint`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId, amount, txHash })
            });
            if (!response.ok) throw new Error('Failed to mint ETF shares');
            return await response.json();
        } catch (error) {
            console.error('Error minting ETF shares:', error);
            throw error;
        }
    }
     ...
     * Redeem ETF shares
     */
    async redeemEtfShares(basketId, userId, shares) {
        try {
            const response = await fetch(`http://localhost:3001/api/etf/${basketId}/redeem`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId, shares })
            });
            if (!response.ok) throw new Error('Failed to redeem ETF shares');
            return await response.json();
        } catch (error) {
            console.error('Error redeeming ETF shares:', error);
            throw error;
        }
    }
     ...
     * Get or create user
     */
    async getOrCreateUser(address, displayName) {
        try {
