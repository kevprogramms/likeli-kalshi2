...
/// Second step: Initialize vault accounts (share mint, vault ATA, manager fee ATA)
/// This is separated because we need the fund_state to exist first for PDA derivation
#[derive(Accounts)]
...
pub fn handler_init_vault_accounts(ctx: Context<InitializeVaultAccounts>) -> Result<()> {
    ...
    msg!("Share mint: {}", fund.share_mint);
